...
fn test_function_calls() {
    let input =
        r#"*[_type=="article"]{title,slug,upper(title),lower(excerpt),string::split(tags,",")}"#;
    let expected = r#"*[_type == "article"] {
  title,
  slug,
  upper(title),
...
#[test]
fn test_custom_function_single() {
    let input = r#"fn pt::isPublished($doc) = !(_id in path('drafts.**')); *[pt::isPublished(@)]"#;
    let expected = r#"fn pt::isPublished($doc) = !(_id in path("drafts.**"));
...
#[test]
fn test_custom_function_multiple() {
    let input = r#"fn math::double($n) = $n * 2; fn math::triple($n) = $n * 3; *"#;
    let expected = "fn math::double($n) = $n * 2;\nfn math::triple($n) = $n * 3;\n\n*";
...
#[test]
fn test_custom_function_with_projection() {
    let input = r#"fn my::full($p) = $p.first + " " + $p.last; *[_type=="person"]{"fullName": my::full(@)}"#;
    let expected = r#"fn my::full($p) = $p.first + " " + $p.last;
...
#[test]
fn test_no_custom_functions() {
    // Ensure queries without custom functions still work identically
    let input = r#"*[_type == "post"]"#;
    let expected = r#"*[_type == "post"]"#;
